...
proc readpgm_cond(fname, height, width, D, array : [D] real, min, max) {
...
proc readpgm_temp(fname, height, width, D, array : [D] real, min, max) {
  writeln("Reading PGM image.");
  var f = open(fname, iomode.r).reader();
  ...
  for j in 1..width {
    array[1,j] = array[2,j];
    array[height,j] = array[height-1,j];
  }
}
